...
import AudioToolbox
    ...
    let userDefaults2:UserDefaults = UserDefaults.standard
    ...
    func setupGround() {
        ...
        let moveGround = SKAction.moveBy(x: -groundTexture.size().width , y: 0, duration: 3.0)
    ...
    func setupItem() {
        ...
        let itemTexture = SKTexture(imageNamed: "bill")
        
        let cw = [itemTexture.size().width].max()
        ...
        let createItemAnimation = SKAction.run({
            ...
            item.zPosition = -50.0
            let bill = SKSpriteNode(texture: itemTexture)
            bill.name = "bill"
            bill.setScale(1.0)
            bill.position = CGPoint(x:0.0, y:self.frame.size.height / 2)
            bill.physicsBody = SKPhysicsBody(texture: bill.texture!,size: bill.texture!.size())
            bill.physicsBody?.isDynamic = false
            bill.physicsBody?.categoryBitMask = self.itemCategory
            bill.physicsBody?.contactTestBitMask = self.birdCategory
            item.addChild(bill)
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyB.categoryBitMask & scoreCategory) == scoreCategory {
            ...
            print("Item ScoreUp")
            print("What bodyA is = \(contact.bodyA.node!.name)")
            print("What bodyB is = \(contact.bodyB.node!.name)")
            if contact.bodyA.node?.name == "bill" {
          contact.bodyA.node?.removeFromParent()
            }
            if contact.bodyB.node?.name == "bill" {
                contact.bodyB.node?.removeFromParent()
            }

          iscore += 1
          itemScoreLabelNode.text = "Item Score:\(iscore)"
            // ベストスコア更新か確認する --- ここから ---
          var bestItemScore = userDefaults2.integer(forKey: "BESTITEM")
          if iscore > bestItemScore {
          bestItemScore = iscore
          bestItemScoreLabelNode.text = "Best Item Score:\(bestItemScore)"
          userDefaults2.set(bestItemScore, forKey: "BESTITEM")
    ...
    func restart() {
        ...
        itemScoreLabelNode.text = String("Item Score:\(iscore)")
        bird.position = CGPoint(x: self.frame.size.width * 0.2, y:self.frame.size.height * 0.7)
        bird.physicsBody?.velocity = CGVector.zero
        bird.physicsBody?.collisionBitMask = groundCategory | wallCategory
        bird.zRotation = 0.0
    ...
    func setupItemScoreLabel() {
        ...
        let bestItemScore = userDefaults2.integer(forKey: "BESTITEM")
        bestItemScoreLabelNode.text = "Best Item Score:\(bestItemScore)"
